/*------------------------------------------------------------------------------
  ...
  2021/08/09  OK      pr_API_USPS_AddShipmentRequest: Bugfix to get the distinct order information when order has multiple shipments (BK-478)
  2021/07/29  OK      pr_API_USPS_AddShipmentRequest: Changes to add update waveno (BK-445)
------------------------------------------------------------------------------*/

Go

if object_id('dbo.pr_API_USPS_AddShipmentRequest') is not null
  drop Procedure pr_API_USPS_AddShipmentRequest;
Go
/*------------------------------------------------------------------------------
  Proc pr_API_USPS_AddShipmentRequest:
...
------------------------------------------------------------------------------*/
Create Procedure pr_API_USPS_AddShipmentRequest
  (@InputXML     TXML,
...
begin /* pr_API_USPS_AddShipmentRequest */
  SET NOCOUNT ON;
  ...
  insert into APIOutboundTransactions (IntegrationName, MessageType, EntityType, EntityId, EntityKey, Reference1, BusinessUnit, CreatedBy)
    select 'CIMSENDICIAUSPS', 'ShipmentRequest', @vEntityType, EntityId, EntityKey, UDF1, @BusinessUnit, @UserId
    from @ttEntitiesToCreateShipment

ExitHandler:
  return(coalesce(@vReturnCode, 0));
end /* pr_API_USPS_AddShipmentRequest */
