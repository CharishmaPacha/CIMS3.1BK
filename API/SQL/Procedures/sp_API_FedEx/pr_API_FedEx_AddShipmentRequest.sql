/*------------------------------------------------------------------------------
  ...
  2021/08/09  OK      pr_API_FedEx_AddShipmentRequest: Bugfix to get the distinct order information when order has multiple shipments (BK-478)
  2021/07/29  OK      pr_API_FedEx_AddShipmentRequest: Changes to add update waveno (BK-445)
------------------------------------------------------------------------------*/

Go

if object_id('dbo.pr_API_FedEx_AddShipmentRequest') is not null
  drop Procedure pr_API_FedEx_AddShipmentRequest;
Go
/*------------------------------------------------------------------------------
  Proc pr_API_FedEx_AddShipmentRequest:
    Generates API outbound transaction to create FEDEX shipments. This procedure using
    #ShipLabelsToInsert to get the different PTs and add API out bound transactions.
    Also we are always generating the shipment with respect to the PickTicket

  #ShipLabelsToInsert - TShipLabels
------------------------------------------------------------------------------*/
Create Procedure pr_API_FedEx_AddShipmentRequest
  (@InputXML     TXML,
...
begin /* pr_API_FedEx_AddShipmentRequest */
  SET NOCOUNT ON;
  ...
  if (object_id('tempdb..#ShipLabelsToInsert') is not null)
    begin
      /* For FEDEX we are creating shipment with multiple package for the Order. If we have already
  ...
  if not exists(select * from @ttEntitiesToCreateShipment) and (@vEntityType is not null)
    insert into @ttEntitiesToCreateShipment(EntityId, EntityKey)
      select @vEntityId, @vEntityKey
  ...
  insert into APIOutboundTransactions (IntegrationName, MessageType, EntityType, EntityId, EntityKey, Reference1, BusinessUnit, CreatedBy)
    select 'CIMSFEDEX', 'ShipmentRequest', @vEntityType, EntityId, EntityKey, UDF1, @BusinessUnit, @UserId
    from @ttEntitiesToCreateShipment

ExitHandler:
  return(coalesce(@vReturnCode, 0));
end /* pr_API_FedEx_AddShipmentRequest */
