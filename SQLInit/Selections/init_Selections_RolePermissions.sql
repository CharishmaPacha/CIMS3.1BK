/*------------------------------------------------------------------------------
  ...
  2020/04/18  TK      Initial revision (HA-68)
------------------------------------------------------------------------------*/

Go

declare @ContextName           TName = 'List.RolePermissions',
        @SelectionName         TName,
...
insert into @ttSelectionFilters
      ...
      select 'D',        'IsVisible',        'Equals',           '1',                'N';

exec pr_Setup_Selections @ContextName, @SelectionName, @SelectionDescription, 'D' /* Selection Type  */, default /* Visible */, @ttSelectionFilters;

/******************************************************************************/
delete from @ttSelectionFilters;
select @SelectionName        = 'UI-Roles', -- Default
       @SelectionDescription = 'CIMS UI';

insert into @ttSelectionFilters
            (FilterType, FieldName,          FilterOperation,    FilterValue,        Visible)
      select 'M',        'Application',      'Equals',           'CIMS.UI',          'Y'
union select 'D',        'RoleDescription',  'StartsWith',       'Admin',            'Y'

exec pr_Setup_Selections @ContextName, @SelectionName, @SelectionDescription, 'D' /* Selection Type- User */, default, @ttSelectionFilters;

/******************************************************************************/
delete from @ttSelectionFilters;
select @SelectionName        = 'RF-Roles', -- Default
       @SelectionDescription = 'CIMS RF';

insert into @ttSelectionFilters
            (FilterType, FieldName,          FilterOperation,    FilterValue,        Visible)
      select 'M',        'Application',      'Equals',           'CIMS.RF',          'Y'
union select 'D',        'RoleDescription',  'StartsWith',       'Admin',            'Y'
