/*------------------------------------------------------------------------------
  ...
  2020/12/15  KBB     pr_CycleCount_Action_AssignUser: Added New Action Procedure for Cctasks assign to user (HA-1792)
------------------------------------------------------------------------------*/

Go

if object_id('dbo.pr_CycleCount_Action_AssignUser') is not null
  drop Procedure pr_CycleCount_Action_AssignUser;
Go
/*------------------------------------------------------------------------------
  Proc pr_CycleCount_Action_AssignUser: This procedure is used to Assign the users
------------------------------------------------------------------------------*/
Create Procedure pr_CycleCount_Action_AssignUser
  (@xmlData       xml,
...
begin /* pr_CycleCount_Action_AssignUser */
  SET NOCOUNT ON;
  ...
  select @vReturnCode     = 0,
         ...
         @vRecordId       = 0,
         @vRecordsUpdated = 0,
         @vAuditActivity  = 'AT_CCTaskassigned';
  ...
  /* If CycleCountTasks status is of invalid status to assign to a user then delete them from #table */
  delete ttSE
  output 'E', 'CycleCountTasks_AssignUser_InvalidStatus', T.BatchNo
...
end /* pr_CycleCount_Action_AssignUser */
